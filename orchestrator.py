...
def ejecutar_pipeline_para_un_video(intentar_publicar: bool, generar_short: bool):
    ...
    # DOBLE VETO (02-sep-2026, fallo real: el filtro aprobó la IDEA pero el
    # guionista tituló "Frecuencias Para Dormir..." — tema en cuarentena —
    # y se publicó). El veto ahora se re-aplica al TÍTULO FINAL del guion.
    if _tema_esta_vetado(guion.get("titulo", "")):
        log(AGENT, f"ABORTADO: el título final del guion cae en el TEMA VETADO "
                   f"(cuarentena música/sonidos/frecuencias): '{guion.get('titulo','')}'. "
                   f"Mejor ningún video que otro video del tema borrado 2 veces.")
        return None

    # BARRERA DURA DE DURACIÓN (02-sep-2026, fallo real: se publicó un largo
    # de 8m25s con mínimo configurado de 16). Si tras todas las extensiones
    # del guionista el guion sigue corto en más de 2 minutos, se ABORTA:
    # el horario de respaldo (18:15) reintenta con otro tema, y un video de
    # 8 min NO cumple la estrategia de monetización (12-26 min).
    _palabras_guion = sum(len((b.get("texto") or "").split())
                          for c in guion.get("capitulos", [])
                          for b in c.get("beats", []))
    _min_estrategia = cfg["estrategia"]["duracion_minima_min"]
    _dur_estimada = _palabras_guion / 185.0  # palabras/min reales de la voz
    if _dur_estimada < (_min_estrategia - 2):
        log(AGENT, f"ABORTADO: el guion quedó en ~{_dur_estimada:.1f} min "
                   f"(mínimo {_min_estrategia}). No se publica un largo corto; "
                   f"el horario de respaldo reintentará con otro tema/LLM.")
        return None
